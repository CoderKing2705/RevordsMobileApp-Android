...
const Favourite = ({ navigation }) => {
  ...
  const images = [{ url: announcementImage }];
  ...
  const setIsAnnouncementModalVisibleData = async (
    announcement,
    businessdata
  ) => {
      ...
      await axios({
        ...
        .catch(async (error) => {
          await useErrorHandler(
            "(Android): Favourites > closeRedeemModal(): " + error
          );
          setLoading(false);
        });
    } catch (error) {
  ...
  const getRefreshData = async () => {
    try {
      AsyncStorage.getItem("token")
          ...
          if (value !== null) {
            await axios({
              ...
              await Geolocation.getCurrentPosition(
                ...
                async (error) => {
                  await useErrorHandler(
                    "(Android): Favourites > refreshedData(): " + error
                  );
                },
        ...
        .catch(async (error) => {
          await useErrorHandler(
            "(Android): Favourites > getRefreshedData(): " + error
          );
          setLoading(false);
        });
    } catch (error) {
  ...
  const checkNotificationPermission = async () => {
    try {
      ...
      await useErrorHandler(
        "(Android): Favourites > checkNotificationPermission(): " + error
      );
    }
  };
          ...
          if (value !== null) {
            ...
            await fetch(
              ...
              .catch(async (error) => {
                ...
                await useErrorHandler(
                  "(Android): Favourites > likeProfile() " + error
                );
                setLoading(false);
              });
          } else {
                  ...
                  wishList.map((item, index) => (
                    <View
                      ...
                      style={[
                        ...
                        isAutoPilotModalVisible ? { opacity: 0.4 } : "",
                      ...
                      >
                        <View style={styles.cardView}>
                          {item.promotionData.map((promotion, earnReward) => (
                            ...
                            >
                              <Text style={styles.achievableName}>
                                {promotion.promotionalMessage}
                              </Text>
                              {promotion.expiryDays > 1 && (
                                <Text style={styles.achievalbeValue}>
                              ...
                              <TouchableOpacity
                                activeOpacity={0.7}
                                onPress={() => openPromoModal(promotion, item)}
                                style={[
                                  promotion.isClaimed == false
                                    ? styles.frame2vJuClaim
                                    : styles.frame2vJuClaimed,
                                ]}
                              >
                                {promotion.isClaimed == false && (
                          ...
                          {item.autopilotData.map((autopilot, earnReward) => (
                            ...
                            >
                              ...
                              <TouchableOpacity
                                activeOpacity={0.7}
                                onPress={() => openAPModal(autopilot, item)}
                                style={[
                                  autopilot.isClaimed == false
                                    ? styles.frame2vJuClaim
                                    : styles.frame2vJuClaimed,
                                ]}
                              >
                          ...
                          {item.announcementData.map(
                            (announcement, earnReward) => (
                              <Card
                                key={earnReward}
                                style={[
                                  {
                              ...
                              >
                                ...
                                <TouchableOpacity
                                  activeOpacity={0.7}
                                  onPress={() =>
                                    openAnnouncementModal(announcement, item)
                                  }
                                  style={styles.ViewAnnouncement}
                                >
                          ...
                          {item.rewardData.map((reward, earnReward) => (
                            ...
                            >
                              ...
                              <Text style={styles.achievalbeValue}>
                                {reward.achivableTargetValue} pts
                              </Text>
                              <View>
                                <Progress.Bar
                                  ...
                                  progress={
                                    1 -
                                    reward.pendingToAchiveValue /
                                      reward.achivableTargetValue
                                  }
                                  width={110}
                                  color="#2ac95d"
                                />
                              </View>
                              {reward.pendingToAchiveValue > 0 && (
                                <Text style={styles.pendingpoints}>
                                  {reward.pendingToAchiveValue} left
                                </Text>
                              )}
                              {reward.pendingToAchiveValue <= 0 && (
                                <Text style={styles.pendingpoints}>0 left</Text>
                              )}
                            </Card>
                          ))}
                        </View>
        ...
        >
          <View style={styles.modal}>
            ...
            <Text style={styles.modalbusinessName}>
              {businessClaimData.businessName}
            </Text>
            <Text style={styles.modalPromoMsg}>
              DEAL : {promotionClaimData.promotionalMessage}
            </Text>
            <Text style={styles.modaltext}>
              <Text style={{ fontWeight: "700" }}>Offer Start Date </Text>-{" "}
              {moment(promotionClaimData.offerStartDate).format("MM/DD/YYYY")}
            </Text>
            <Text style={styles.modaltext}>
              <Text style={{ fontWeight: "700" }}>Offer End Date </Text>-{" "}
              {moment(promotionClaimData.offerEndDate).format("MM/DD/YYYY")}
            </Text>
            {promotionClaimData.expiryDays > 1 && (
              <Text style={styles.modaltext}>
                <Text style={{ fontWeight: "700" }}>Expires in </Text>-{" "}
                {promotionClaimData.expiryDays} days
              </Text>
            )}
            {promotionClaimData.expiryDays == 1 && (
              <Text style={styles.modaltext}>
            ...
            {promotionClaimData.isSpinWheel && (
              <Text style={styles.modaltext}>
                Spin the wheel and get rewards at store
              </Text>
            )}

            {promotionClaimData.filePath != "" &&
              promotionClaimData.filePath != null && (
                <Image
                  style={styles.avatarImg}
                  source={{ uri: promotionClaimData.filePath }}
                  resizeMode="stretch"
                ></Image>
              )}
            <Text style={styles.modaltext}>
              ...
              <Text style={{ fontWeight: "700" }}>
                {" "}
                {promotionClaimData.redeemableAt}
              </Text>
            </Text>
            {promotionClaimData.isClaimed == false && (
              <TouchableOpacity
                activeOpacity={0.7}
                onPress={() => closeRedeemModal("promo", promotionClaimData.id)}
      ...
      >
        ...
        >
          <View style={styles.modal}>
            ...
            <Text style={styles.modalPromoMsg}>
              {autoPilotClaimData.rewardName}
            </Text>
            {autoPilotClaimData.expiryDays > 1 && (
              <Text style={styles.modaltext}>
                <Text style={{ fontWeight: "700" }}>Expires in </Text>-{" "}
                {autoPilotClaimData.expiryDays} days
              </Text>
            )}
            {autoPilotClaimData.expiryDays == 1 && (
              <Text style={styles.modaltext}>
            ...
            {autoPilotClaimData.filePath != "" &&
              autoPilotClaimData.filePath != null && (
                <Image
                  style={styles.avatarImg}
                  source={{ uri: autoPilotClaimData.filePath }}
                  resizeMode="stretch"
                ></Image>
              )}
            <Text style={styles.modaltext}>
              Redeemable at -
              <Text style={{ fontWeight: "700" }}> Any Locations</Text>
            </Text>
            {autoPilotClaimData.isClaimed == false && (
              <TouchableOpacity
                activeOpacity={0.7}
                onPress={() =>
                  closeRedeemModal("ap", autoPilotClaimData.historyId)
                }
                style={styles.frame2vJu1ModalClaim}
              >
                <Text style={styles.getStartednru1}>Claim</Text>
      ...
      >
        ...
        >
          <View style={styles.modal}>
            ...
            <Text style={styles.modalPromoMsg}>
              {announcementClaimData.subject}
            </Text>
            {announcementClaimData.validDate && (
              <Text style={[styles.modaltext, { textAlign: "center" }]}>
                <Text style={{ fontWeight: "700" }}>Expiry Date: </Text>
                {moment(announcementClaimData.validDate).format("MM/DD/YYYY")}
              </Text>
            )}

            {announcementClaimData.fileName != "" &&
              announcementClaimData.fileName != null && (
                <TouchableOpacity
                  onPress={() =>
                    handleGalleryImagePress(announcementClaimData.fileName)
                  }
                >
                  <Image
                    style={styles.avatarImg}
                    source={{ uri: announcementClaimData.fileName }}
                    resizeMode="stretch"
                  ></Image>
                </TouchableOpacity>
              )}
